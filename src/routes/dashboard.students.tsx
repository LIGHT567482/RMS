...
import {
  useStore, getStudents, addStudent, updateStudent, deleteStudent, getSubjects, getCombinations, addCombination, deleteCombination,
} from "@/lib/storage";
import {
  ALL_CLASSES, OPTIONAL_SUBJECTS, ORDINARY_LEVELS, ADVANCED_LEVELS, isAdvancedLevel,
  ADVANCED_SUBJECTS, ADVANCED_SUBSIDIARY_SUBJECTS, generateAdvancedCombination, displaySubjectName,
  type ClassLevel, type Gender, type Student, COMPULSORY_SUBJECTS, type Combination,
} from "@/lib/types";
...
function subjectsFromAdvancedComboString(comboText: string): string[] | undefined {
  ...
  const subsidiary = subCode === "ICT" ? "SubsidiaryICT" : subCode === "SM" || subCode === "M" ? "SubsidiaryMath" : undefined;
...
function formatStudentSubjectsForDisplay(student: Student, combinations: Combination[]) {
  ...
  return getStudentCombo(student, combinations) ?? student.enrolledSubjects?.join(", ") ?? "Not set";
}
...
function StudentsPage() {
  ...
  const [showCombinationManager, setShowCombinationManager] = useState(false);
  const [newCombination, setNewCombination] = useState({ name: "", shortForm: "", subjects: [] as string[] });

  const handleAddCombination = () => {
    if (!newCombination.name.trim() || !newCombination.shortForm.trim() || newCombination.subjects.length === 0) {
  ...
  const allSubjectOptions = useMemo(() => {
    return subjects.map((subject) => subject.name).sort();
  }, [subjects]);
  ...
  }, [students, classFilter, levelFilter, genderFilter, subjectFilter, combinationFilter, searchQuery]);
  ...
  const buildExportRows = () => filtered.map((student) => {
    ...
    if (exportFields.lin) row.LIN = student.lin ?? "";
    if (exportFields.registrationNumber) row["Registration Number"] = student.registrationNumber ?? "";
  ...
  const downloadWorkbook = (bookType: "xlsx" | "csv" | "ods") => {
    ...
    const blob = new Blob(
      [bookType === "csv" ? data : new Uint8Array(data as ArrayBuffer)],
  ...
  const validateImportRow = (row: ImportRow): string[] => {
    ...
    if (!row.lin.trim() && !row.registrationNumber.trim()) {
      errors.push("LIN or registration number is required.");
    }
  ...
  const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsedRows = rows.map((row, index) => {
        const rowClass = String(row["Class"] || row["class"] || "").trim();
        const normalizedClass = ALL_CLASSES.includes(rowClass as ClassLevel) ? rowClass as ClassLevel : undefined;
        const rawSubjects = String(row["Subjects"] || row["subjects"] || row["Enrolled Subjects"] || row["Optional Subjects"] || "").trim();
        const gender = normalizeGender(row["Gender"] || row["gender"] || "");
        const importRow: ImportRow = {
          id: `row-${index}`,
          name: String(row["Name"] || row["name"] || "").trim(),
          lin: String(row["LIN"] || row["lin"] || "").trim(),
          registrationNumber: String(row["Registration Number"] || row["registration number"] || row["registrationNumber"] || "").trim(),
          gender,
  ...
  const applyImport = () => {
    ...
    for (const row of selectedRows) {
      ...
      const advancedEnrollment = isAdvanced ? parseAdvancedEnrollmentInput(row.rawSubjects) : undefined;
      const studentData: Omit<Student, "id" | "createdAt"> = {
        name: row.name,
        lin: row.lin || undefined,
        registrationNumber: row.registrationNumber || undefined,
        classLevel,
        gender: row.gender,
        optionalSubjects: isAdvanced ? undefined : importSubjects.filter((subject) => OPTIONAL_SUBJECTS.includes(subject)),
      ...
      const existing = students.find((student) =>
        (student.lin && student.lin === row.lin) ||
    ...
    setImportRows([]);
    toast.success(`${selectedRows.filter((row) => !row.errors.length).length} student(s) imported.`);
  };
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <Card className="p-4 grid gap-3 sm:grid-cols-[minmax(240px,_1fr)_minmax(240px,_1fr)] items-center">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(exportFields).map(([key, selected]) => (
              <label key={key} className="inline-flex items-center gap-2 text-sm">
                ...
                {key === "name" ? "Name" : key === "lin" ? "LIN" : key === "registrationNumber" ? "Registration Number" : key === "gender" ? "Gender" : key === "classLevel" ? "Class" : "Subjects"}
              </label>
            ))}
          </div>
        </div>
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Use `.xlsx`, `.csv`, or `.ods` files. Required columns: Name, LIN or Registration Number, Gender, Class, Subjects.
          </p>
        </div>
      </Card>
      ...
      <div className="flex gap-2 items-end flex-wrap">
        ...
        <div className="min-w-32">
          ...
          <Select value={levelFilter} onValueChange={(v) => setLevelFilter(v as "All" | "Ordinary" | "Advanced")} disabled={classFilter !== "All"}>
            <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="min-w-40">
          ...
          <Input
            ...
            placeholder="Search name, LIN, registration"
        ...
        <div className="min-w-40">
          ...
          <Select value={subjectFilter} onValueChange={(v) => setSubjectFilter(v)}>
            ...
            <SelectContent>
              <SelectItem value="All">All</SelectItem>
              {allSubjectOptions.map((sub) => <SelectItem key={sub} value={sub}>{sub}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="flex gap-2">
        <Button variant="outline" size="sm" onClick={() => setShowCombinationManager(!showCombinationManager)}>
          Manage Combinations
        </Button>
      </div>

      {showCombinationManager && (
        <Card className="p-4">
          <div className="space-y-4">
            <h3 className="font-semibold">Manage Advanced Level Combinations</h3>
            <div className="grid gap-3 sm:grid-cols-3">
              <div>
                <Label className="text-xs mb-2 block">Combination Name</Label>
                <Input
              ...
              <div>
                <Label className="text-xs mb-2 block">Short Form</Label>
                <Input
                  value={newCombination.shortForm}
                  onChange={(e) => setNewCombination({ ...newCombination, shortForm: e.target.value.toUpperCase() })}
                  placeholder="e.g., PCM"
                  className="h-8 text-sm"
                />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {ADVANCED_SUBJECTS.concat(ADVANCED_SUBSIDIARY_SUBJECTS).map((subject) => (
                    <label key={subject} className="inline-flex items-center gap-1 text-xs">
                      <Checkbox
                        checked={newCombination.subjects.includes(subject)}
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setNewCombination({ ...newCombination, subjects: [...newCombination.subjects, subject] });
                          } else {
                            setNewCombination({ ...newCombination, subjects: newCombination.subjects.filter(s => s !== subject) });
                          }
                        }}
                      />
                      {subject}
            ...
            <Button size="sm" onClick={handleAddCombination}>
              <Plus className="h-4 w-4 mr-1" /> Add Combination
            </Button>
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {combinations.map((combo) => (
                  <div key={combo.id} className="flex items-center justify-between p-2 border rounded">
                    <div>
                      <p className="text-sm font-medium">{combo.shortForm}</p>
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  ...
                  <th className="p-3 text-left">LIN</th>
                ...
                <tr key={s.id} className="border-t hover:bg-accent/10">
                  ...
                  <td className="p-3">
                    {ORDINARY_LEVELS.includes(s.classLevel)
                      ...
                      : (getStudentCombo(s) ?? s.enrolledSubjects?.join(', ')) || 'Not set'}
                  </td>
                  <td className="p-3">
                    <div className="flex gap-2">
    ...
    <DialogContent className="max-w-lg max-h-screen overflow-y-auto">
      ...
      <form id="student-form" onSubmit={(e) => { e.preventDefault(); save(); }} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label>LIN</Label>
            <Input value={lin} onChange={(e) => setLin(e.target.value)} />
          </div>
          <div>
