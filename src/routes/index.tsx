...
import { getSchool, setSchool, useStore } from "@/lib/storage";
...
function WelcomePage() {
  ...
  function handleForgot(e: React.FormEvent) {
    ...
    const okEmail =
      info.recoveryEmail && info.recoveryEmail.toLowerCase() === recoverEmail.toLowerCase();
    ...
    if (!((okEmail && okPw) || okAns)) {
      return toast.error(
        "Verification failed. Provide the correct email + password OR security answer.",
      );
    }
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-[2fr_1fr] bg-background">
      {/* Left: hero */}
      <div
        className="relative hidden lg:flex items-center justify-center p-12 text-white overflow-hidden"
        style={{
          background: school.signInBackgroundUrl
            ? undefined
            : "linear-gradient(135deg, #f6d34f 0%, #f3b533 50%, #d7891c 100%)",
          backgroundImage: school.signInBackgroundUrl
            ? `url(${school.signInBackgroundUrl})`
            : undefined,
          backgroundSize: school.signInBackgroundUrl ? "cover" : undefined,
          backgroundPosition: school.signInBackgroundUrl ? "center" : undefined,
        }}
      >
        {school.signInBackgroundUrl ? (
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center gap-8 max-w-lg">
          <div>
            <h1 className="font-display text-5xl leading-tight whitespace-nowrap">{school.name}</h1>
            <p className="mt-4 text-white/80">
              {school.motto ||
                "A complete offline platform for managing student records, marks, and printable report cards."}
            </p>
          </div>
          ...
          <div className="w-full max-w-md mx-auto flex flex-col justify-center h-full">
            ...
            <div className="flex flex-col items-center gap-4 mb-6 text-center">
              ...
              <div className="flex items-center justify-center gap-2">
                <h2 className="font-display text-2xl">{school.name}</h2>
              </div>
            </div>
            ...
            {mode === "signin" && (
              <form onSubmit={handleSignIn} className="space-y-4 text-center">
                <div>
                  <Label htmlFor="code" className="mx-auto">
                    Access Code (5–10 digits)
                  </Label>
                  <Input
                    id="code"
                    inputMode="numeric"
              ...
              <form onSubmit={handleSetup} className="space-y-4">
                ...
                <div className="border-t pt-4 space-y-3">
                  <p className="text-xs font-medium text-muted-foreground">
                    RECOVERY (if you forget your code)
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g. School founding year?"
                      />
                    </div>
                    <div>
      ...
      <div className="fixed bottom-4 left-4 text-xs uppercase tracking-[0.2em] text-white z-40 font-semibold">
        LIGHT TECHNOLOGIES
      </div>
    </div>
  );
