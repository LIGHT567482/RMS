...
import { useStore, getStudents, getSubjects, getMarks, getProjects, getSchool } from "@/lib/storage";
...
function Stat({ icon: Icon, label, value, to }: { icon: any; label: string; value: number | string; to: string }) {
  return (
    <Link to={to}>
      <Card className="p-5 hover:shadow-lg transition-all group cursor-pointer h-full"
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{school.name} • {school.motto}</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Stat icon={ClipboardList} label="Mark Entries" value={marks.length} to="/dashboard/marks" />
        <Stat icon={Sparkles} label="Project Records" value={projects.length} to="/dashboard/project-work" />
      </div>

      <Card className="p-6" style={{ background: "var(--gradient-card)" }}>
        <h2 className="font-display text-xl mb-3">Quick Actions</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <Link to="/dashboard/students" className="p-4 rounded-lg border hover:border-primary hover:bg-accent/30 transition">
            <Users className="h-5 w-5 text-primary mb-2" />
            <p className="font-medium">Enroll a student</p>
            <p className="text-xs text-muted-foreground">Add new students by class</p>
          </Link>
          <Link to="/dashboard/marks" className="p-4 rounded-lg border hover:border-primary hover:bg-accent/30 transition">
          ...
          <Link to="/dashboard/reports" className="p-4 rounded-lg border hover:border-primary hover:bg-accent/30 transition">
